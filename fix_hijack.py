...
def fix_port_hijack():
    ...
    try:
        ...
        command = """
        echo "STOPPING DEPLOY USER PROCESSES HIJACKING PORT 3000..."
        su - deploy -c "pm2 stop all"
        
        echo "ENSURING PORT 3000 IS FREE..."
        fuser -k 3000/tcp || true
        fuser -k 3002/tcp || true
        
        echo "RESTARTING EDUSYNC-WEB ON PORT 3000 UNDER ROOT..."
        cd /opt/edusyncai/edusync-web || exit 1
        pm2 delete edusync-web
        pm2 start npm --name "edusync-web" -- start -- -p 3000
        pm2 save
        
        echo "FIXING NGINX PROXY PASS BACK TO PORT 3000 (JUST IN CASE)..."
